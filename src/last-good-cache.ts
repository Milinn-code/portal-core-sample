...
//
// fetcher に求める約束（キャッシュ側では検査しない）:
// - 失敗は、undefined や null ではなく例外で伝える。例外を投げずに返した値は「正常な結果」として扱う。
// - 取得処理自身がタイムアウトを持つ（DB の statement_timeout や HTTP クライアントのタイムアウトなど）。
//   fetcher が終わらないと、同時に待っている get() もすべて終わらない。適切な待ち時間は、
//   取得の中身を知っている fetcher 側で決める。
