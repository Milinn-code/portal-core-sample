...
- 図には管理者が手動で起こせる遷移だけを描いています。日付による自動遷移（`rookie → active`、`leaving → left`）は定期実行ジョブ側で扱うので、ここには含めていません。
...
- **`Number()` の暗黙変換を信用しない。** `Number('')` は `0`、`Number('0x22')` は `34` になります。そのため、10進数の正規表現で先に弾いています。これをしないと、`near=34.5,` が「経度 0」として検索されます。
